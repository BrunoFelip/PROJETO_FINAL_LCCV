                    ...
                    for j in range(numplan):
                        if i!=j:
                            ...
                            if distplan<somRaios:
                                ...
                                dX=0
                            forca[i]=forca[i]+G*massa[i]*massa[j]*(r[j]-r[i])/((np.linalg.norm(r[j]-r[i]))**3)+a[4][i][j]*dX
    ...
    if modelo=='2d':
        ...
        for i in range(len(linha)):
             for j in range(numplan):
                  ...
                  ak.plot(x[j][0:i],y[j][0:i],color='black',mew='2')
                  ak.plot(x[j][i],y[j][i],'ro',mew='0.5')
                  plt.ylim(-1.5*maximY0,1.5*maximY0)
                  plt.xlim(-1.5*maximX0,1.5*maximX0)
                  plt.ion()
                  plt.pause(0.05)
             plt.cla()
        plt.close()
        ...
        fig = plt.figure('Planetas')
        al = fig.gca(projection='3d') #Melhorou a visibilidade dos pontos, mas bugou o texto do tempo.
        for i in range(numplan):
             datax=[]
             datay=[]
             dataz=[]
        ...
        for i in range(len(linha)):
             for j in range(numplan):
#                  fig=plt.figure('Planetas') Pior situação de visualização, pontos plotados de maneira imagem por imagem, fica ruim de identificar.
#                  al=plt.axes(projection='3d') Pior situação de visualização
                  al.plot(x[j][0:i],y[j][0:i],z[j][0:i],color='black',mew='2')
                  al.plot([x[j][i]],[y[j][i]],[z[j][i]],'ro',mew='0.5')
                  al.set_xlim3d(-2*maximX0,2*maximX0)
                  al.set_ylim3d(-2*maximY0,2*maximY0)
                  al.set_zlim3d(-2*maximZ0,2*maximZ0)
                  plt.title('Tempo: {} s'.format(temp[i]))
                  plt.ion()
                  plt.pause(0.05)
             plt.cla()
